...
class ZnicL3RestServerPool(proxy_server_manager.ServerPool):
    """Znic L3 Rest Server Pool for Znic L3 Agent.

    This server pool has the router and floatingip operations
    of create, update and delete, to the Znic Controller.
    """
    def __init__(self, servers, auth,zenic_version, ssl, no_ssl_validation, ssl_sticky,
                 ssl_cert_directory, consistency_interval, timeout=False,
                 cache_connections=True,
                 base_uri='/restconf/operations/zenic-vdcapp-model:',
                 success_codes=SUCCESS_CODES,
                 failure_codes=FAILURE_CODES, name='ZnicL3RestProxy'):
        super(ZnicL3RestServerPool, self).__init__(servers, auth, ssl,
                 no_ssl_validation, ssl_sticky, ssl_cert_directory,
                 consistency_interval, timeout, cache_connections, base_uri,
                 success_codes, failure_codes, name)
        version = zenic_version.split('.')
        version = version[0] + version[1]
        LOG.debug(_("zenic_version = %s"),version)
        if (version.isdigit() == False) or (int(version) < 403):
        ...
        if action == 'DELETE' or action == 'GET':
            ...
            if internal_interfaces:
                internal_interfaces_subnets =\
                    [intf['subnets'][0] for intf in internal_interfaces]
            ...
            a = [subnet['id'] for subnet in internal_interfaces_subnets]
            ...
            if self.zenic_version > 403:
                if '_floatingips' in router_info:
                    routes_floaintips = router_info.get('_floatingips', set())
                    floating_ip = []
                    for routes_floaintip in routes_floaintips:
                        if 'host' in routes_floaintip:
                            del routes_floaintip['host']
                        floating_ip.append(routes_floaintip)
    ...
    def rest_update_router(self, router_info):
        ...
        errstr = "Unable to create remote router: %s"
        resp = self.rest_action('POST', resource, data, errstr)
        if resp[0] == 0:
            return False
        return True
    ...
    def rest_delete_router(self, router_id):
        ...
        resource = 'del-router'
        errstr = "Unable to delete remote router: %s"
        resp = self.rest_action('POST', resource, data, errstr)
        if resp[0] == 0:
            return False
        return True

    def construct_all_routers_rest_msg(self, router_info):
        router_rest_data = {
            "input": {
...
def get_restproxy_conf():
    conf_proxy = cfg.ConfigOpts()
    conf_proxy.register_opts(restproxy_opts, 'RESTPROXY')
...
class ZenicAgent(manager.Manager):
        ...
        if routers:
            ...
            try:
                with lockutils.lock("zenic_agent_update_router"):
                    LOG.info('routers_updated Got update mutex')
    ...
    def _sync_all_router_restconf(self, context):
        ...
        routers = set(router['id'] for router in routers)
        LOG.debug('zenic_agent, starting, get_all_routers:{0}'.format(routers))
        self.updated_routers.update(routers)
    ...
    def _process_routers_loop(self):
        LOG.debug("liuyong Starting _process_routers_loop")
    ...
    def _process_router_update(self):
        ...
        try:
            if self.updated_routers:
                ...
                try:
                    with lockutils.lock("zenic_agent_update_router"):
                        LOG.info('_process_router_update Got update mutex')
                        now_processed = list(self.updated_routers)
                        router_ids = list(self.updated_routers)
                        self.updated_routers.clear()
                except Exception as e:
                    LOG.error("lockutils, except:%s", str(e))
                ...
                for i in range(0, len(router_ids), self.sync_routers_chunk_size):
                    try:
                        now_processed_feched = now_processed[i:i + self.sync_routers_chunk_size]
                        time_start_get_router = time.time()
                        routers = self.plugin_rpc.get_routers(self.context, router_ids[i:i + self.sync_routers_chunk_size])
                    except oslo_messaging.MessagingTimeout:
                        try:
                            time.sleep(5)
                            routers = self.plugin_rpc.get_routers(
                                self.context, router_ids[i:i + self.sync_routers_chunk_size])
                        except oslo_messaging.MessagingTimeout:
                            LOG.error("Second time out error!!!,router_ids:%s" % router_ids)
                            continue
                    ...
                    try:
                        with lockutils.lock("zenic_agent_remove_router"):
                            self.removed_routers.update(set(now_processed_feched) - fetched)
                    except Exception as e:
                        LOG.error("lockutils, except:%s", str(e))

                    time_start_proc_router = time.time()
                    self._process_routers(routers)
                    LOG.info("Zenic ended _process_routers %d routers, get time: %s, proc time: %s"
                               % (len(fetched), (time_start_proc_router-time_start_get_router), (time.time()- time_start_proc_router)))
            self._process_router_delete()
        except Exception as e:
            LOG.exception(_("Failed synchronizing routers: {}".format(e)))
            self.fullsync = True
...
def main(manager='neutron.agent.zenic_agent.ZenicAgentWithStateReport'):
