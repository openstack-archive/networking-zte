...
class ServerProxy(object):
    ...
    def rest_call(self, action, resource, data='', headers={}, timeout=False,
        ...
        LOG.info(_("ServerProxy: server=%(server)s, port=%(port)d, "
                    "ssl=%(ssl)r"),
                  {'server': self.server, 'port': self.port, 'ssl': self.ssl})
        LOG.info(_("ServerProxy: resource=%(resource)s, data=%(data)r, "
...
class ServerPool(object):
    ...
    def rest_action(self, action, resource, data='', errstr='%s',
        ...
        """
        LOG.info(_("rest_action:"
                   "%(action)s action to resource %(resource)s %(data)s"),
